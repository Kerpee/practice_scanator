...
class Binarizer:
    ...
    def process(self, img: np.ndarray) -> tuple:
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (self.tophat_ksize, self.tophat_ksize))
        tophat = cv2.morphologyEx(equalized, cv2.MORPH_TOPHAT, kernel)
        _, binary_cropped = cv2.threshold(tophat, self.thresh_value, 255, cv2.THRESH_BINARY)
        full_binary_mask = np.zeros(img.shape[:2], dtype=np.uint8)
        full_binary_mask[offset_y:offset_y + binary_cropped.shape[0],
        offset_x:offset_x + binary_cropped.shape[1]] = binary_cropped
