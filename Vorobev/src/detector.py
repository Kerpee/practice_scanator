...
def find_cross_rough_center(gray_img):
    ...
    if vert_clean.max() > 0: vert_clean /= vert_clean.max()
    if horiz_clean.max() > 0: horiz_clean /= horiz_clean.max()
    ...
    best_res = _select_best_peak_by_symmetry(peak_candidates, normalized)
    
    if best_res[0] is None:
        steps.append((cv2.cvtColor(gray_img, cv2.COLOR_GRAY2BGR), "Отклонено: Нет симметричных крестов"))
        return None, None, None, normalized, steps
    ...
    cv2.drawMarker(result_vis, (int(best_cx), int(best_cy)), (0, 0, 255), cv2.MARKER_CROSS, 30, 2)
...
def _find_all_peaks(score_map, min_distance=30):
    ...
    while True:
        ...
        if max_val < threshold:
            break
        peaks.append({'x': float(max_loc[0]), 'y': float(max_loc[1]), 'strength': float(max_val)})
        cv2.circle(temp_map, max_loc, min_distance, 0, -1)

    return peaks


def _select_best_peak_by_symmetry(peaks, normalized_img):
    ...
    if best_peak is None or best_score == 0:
