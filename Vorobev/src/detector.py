...
def find_cross_rough_center(gray_img):
    ...
    MIN_INTENSITY = 7
    if vert_lines.max() < MIN_INTENSITY or horiz_lines.max() < MIN_INTENSITY:
        steps.append((np.zeros((h, w, 3), dtype=np.uint8), "4. Карта пересечений (пусто - нет ярких линий)"))
    ...
    best_res = _select_best_peak_by_symmetry(
...
def _select_best_peak_by_symmetry(peaks, normalized_img, vert_map, horiz_map):
    ...
    for peak in peaks:
        ...
        if e_up < MIN_ENERGY_THRESHOLD or e_down < MIN_ENERGY_THRESHOLD or \
           e_left < MIN_ENERGY_THRESHOLD or e_right < MIN_ENERGY_THRESHOLD:
            continue
